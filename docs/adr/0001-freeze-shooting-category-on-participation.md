...
When a member is added to a tir aux plaquettes, the member's usual shooting category is copied onto that participation and may then be adjusted for that tir only. We do this because members can move from tir appuye to tir standard over time, while closed standings must preserve the category that was actually used when the tir happened.
